			       ...
			       size_t count, loff_t *offset)
{
	...
	if (!gpio_get_value(nfc_irq)) {
		...
		//printk("pn544 read1  nfc_irq=%d\n", nfc_irq);			
		//printk("pn544 read2  gpio_get_value(nfc_irq)=%d\n", gpio_get_value(nfc_irq));		

		//printk("fangzhihua pn544 read1  nfc_irq=%d\n", nfc_irq);			

		ret = wait_event_interruptible(pn544_dev->read_wq, gpio_get_value(nfc_irq));
...
static const struct of_device_id pn544_platform_of_match[] = {
	{.compatible = "mediatek,mt6735-nfc",},
	{},
};
			...
			const struct i2c_device_id *id)
{
	...
	*/    
	...
	node = of_find_matching_node(NULL, pn544_platform_of_match);
	if (node) {
		of_property_read_u32_array(node,"debounce", ints, ARRAY_SIZE(ints));
		gpio_set_debounce(ints[0], ints[1]);

		//printk("pn544 debounce:ints[0]=%d,ints[1]=%d", ints[0], ints[1]);
		...
		ret =
		    request_irq(client->irq, pn544_dev_irq_handler,
				IRQF_TRIGGER_HIGH, "nfc_eint_as_int", pn544_dev);
