...
#define VOL_1000 1000000
	...
	if (DUAL_CAMERA_MAIN_SENSOR == SensorIdx)
		...
		pinSetIdx = 2;
#if defined(CONFIG_PROJECT_L7)||defined(CONFIG_PROJECT_T62)
	if (On) {
		...
		if (currSensorName&& (0 == strcmp(SENSOR_DRVNAME_GC2365_MIPI_RAW, currSensorName))) {
			...
			/* VCAM_IO */
			if (TRUE != _hwPowerOnCnt(VCAMIO, VOL_1800, mode_name)) {
				printk("[CAMERA SENSOR] Fail to enable digital power (VCAM_IO), power id = %d\n",VCAMIO);
				goto _kdCISModulePowerOn_exit_;
			}
			/* VCAM_A */
			if (TRUE != _hwPowerOnCnt(VCAMA, VOL_2800, mode_name)) {
			...
			/* AF_VCC */
			if (TRUE != _hwPowerOnCnt(VCAMAF, VOL_2800, mode_name)) {
				printk("[CAMERA SENSOR] Fail to enable analog power (VCAM_AF), power id = %d\n",VCAMAF);
				goto _kdCISModulePowerOn_exit_;
			}
		...
		if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2365_MIPI_RAW, currSensorName))) {
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
				mtkcam_gpio_set(1, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
			}
			/* Set Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
				mtkcam_gpio_set(1, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
			}
			...
			if (TRUE != _hwPowerDownCnt(VCAMAF, mode_name)) {
				...
				if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
				mtkcam_gpio_set(0, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
			}
			/* Set Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
				mtkcam_gpio_set(0, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
			}
...
#elif 0 //defined(CONFIG_PROJECT_M7)
	if (On) {
		ISP_MCLK1_EN(1);

		PK_DBG("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);

		if ((pinSetIdx == 0)&&(currSensorName&& (0 == strcmp(SENSOR_DRVNAME_OV8856_MIPI_RAW, currSensorName)))) {
			//mtkcam_gpio_set(pinSetIdx, CAMLDO, 1);
			/* First Power Pin low and Reset Pin Low */
			PK_DBG("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, "ov8856");
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[1-pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);

			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST,pinSet[1-pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
		 ...
		 if ((pinSetIdx == 1)&&(currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8856_SUB_MIPI_RAW, currSensorName)))) {
			/* First Power Pin low and Reset Pin Low */
			PK_DBG("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, "ov8858sub");
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
				mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[1-pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
		...
		if ((pinSetIdx == 0)&&(currSensorName&& (0 == strcmp(SENSOR_DRVNAME_OV8856_MIPI_RAW, currSensorName)))) {
			...
			/* Set Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
				mtkcam_gpio_set(1, CAMRST, pinSet[0][IDX_PS_CMRST + IDX_PS_OFF]);
			}
			//mtkcam_gpio_set(pinSetIdx, CAMLDO, 0);
			if (TRUE != _hwPowerDownCnt(VCAMD, mode_name)) {
				PK_DBG("[CAMERA SENSOR] Fail to OFF core power (VCAM_D), power id = %d\n", VCAMD);
