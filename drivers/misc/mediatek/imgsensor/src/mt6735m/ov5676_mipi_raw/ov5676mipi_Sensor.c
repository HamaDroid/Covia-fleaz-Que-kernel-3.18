...
#include "ov5676mipi_Sensor.h"

#define PFX "ov5676_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = 0x5675,	//record sensor id defined in Kd_imgsensor.h
	
	.checksum_value = 0xea4c5225,		//checksum value for Camera Auto Test
...
{
	LOG_INF("OV5676_Sensor_Init_2lane E\n");
	//OV5676R1A_AM01d 
...
{
	#if 0
	LOG_INF(" OV5676PreviewSetting_2lane enter\n");
...
{
  #if 0
	LOG_INF("OV5676CaptureSetting_2lane enter! currefps:%d\n",currefps);
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {	
				*sensor_id = 0x5677;			
				printk("===OV5676=== Read sensor id success, i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("===OV5676=== Read sensor id fail, i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
UINT32 OV5676_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV5676_MIPI_RAW_SensorInit	*/
