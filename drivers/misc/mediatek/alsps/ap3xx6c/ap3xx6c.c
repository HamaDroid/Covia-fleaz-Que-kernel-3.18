...
#define Display_Real_CT_Value 	 //Real CT_Value
int CT_Value_Temp[2];//for read 0x28/0x29 to get CT_Value
int CT_Value;
...
static long ap3xx6_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_GET_PS_RAW_DATA:
	        //BY_Devin 20160912
	        #ifdef Display_Real_CT_Value //=1: Real_CT_Value ; 									
				CT_Value_Temp[0] = ap3xx6_read_reg(client, 0x28, 0xFF, 0x00);//read low_8bit
				if (CT_Value_Temp[0] < 0) 
					goto err_out;

				CT_Value_Temp[1] = ap3xx6_read_reg(client, 0x29, 0xFF, 0x00);//read high_1bit  
				if (CT_Value_Temp[1] < 0)                                    //total: 9bit
					goto err_out;

				CT_Value= CT_Value_Temp[0] | (CT_Value_Temp[1]<<8);
				printk("---CT Value = %d\n",CT_Value);
			#else	//=0: CT_Value after Calibration
				CT_Value	=0;
			#endif
			err = ap3xx6_read_ps(obj->client, &obj->ps);
			if (err != 0) {
