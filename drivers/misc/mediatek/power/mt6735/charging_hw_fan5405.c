...
#ifdef CONFIG_HIGHCURRENT_1500MA
static u32 charging_hw_init(void *data)
{
	...
	if (batt_cust_data.high_battery_voltage_support)
	{
		fan5405_reg_config_interface(0x04,0x7A); //1000mA,146mA
	}else
...
static u32 charging_hw_init(void *data)
{
	...
	if (batt_cust_data.high_battery_voltage_support)
	{
		fan5405_reg_config_interface(0x06,0x58);
		fan5405_reg_config_interface(0x02,0xae);
	}else
		fan5405_reg_config_interface(0x06, 0x70);
	...
	if (batt_cust_data.high_battery_voltage_support)
	{
		fan5405_reg_config_interface(0x04,0x5A); //1000mA,146mA
	}else
		fan5405_reg_config_interface(0x04, 0x1A);	/* 146mA */
		charging_init_flag = KAL_TRUE;
	}
